...
async function buildControlData(userId: number, token: string, login: string, useCache: boolean) {
  const cacheKey = `github:dashboard:${userId}`;
  const data = useCache
    ? await cachedRead(cacheKey, CACHE_TTL, () => buildDashboard("github", token, login))
    : await buildDashboard("github", token, login);
  await ensureReviewTables();
  const [reviewStats, jobs] = await Promise.all([
    getReviewStats(userId, "github"),
    listReviewJobs(userId, "github", 10),
  ]);
  return {
    ...data,
    reviewStats,
...
export async function POST(req: NextRequest) {
  ...
  try {
    await cacheDelPattern("github:dashboard:*");
    await cacheDelPattern("github:contributions:*");
