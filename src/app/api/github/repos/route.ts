...
/** 按当前登录用户（GitHub）返回分页仓库 + 全量聚合状态；缓存全量结果，切页命中缓存切片 */
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const all = await cachedRead<GitHubRepoRow[]>(
      `github:repos:${r.ctx.dbUser.id}`,
      60,
      async () => {
