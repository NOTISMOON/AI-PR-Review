...
/** Gitee 提交贡献热力图数据（按 commit 按天聚合，Redis 缓存） */
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const data = await cachedRead(`gitee:contributions:${r.ctx.dbUser.id}:${year ?? "near"}`, CACHE_TTL, () =>
      buildContributions("gitee", r.token!, r.ctx.user.login, {
